...
**336 tests** under `src/tests/`, of which **329 run offline** by default and pass with no
network and no model weights (~20 s). The other **7 are opt-in** and skip unless you ask for
them: 5 need `DL_RUN_NETWORK_TESTS=1`, 2 need `DL_RUN_MODEL_TESTS=1`. Configured in
`pyproject.toml` (`[tool.pytest.ini_options]`: `testpaths=src/tests`, `pythonpath=src`, live
logging on).

> The per-file table below counts **all 336**, including the opt-in ones, so it sums to 336
> rather than 329.
...
| File | Tests | Covers |
|---|---|---|
| `test_config.py` | 9 | defaults, nested env override, validation, cache, `ensure_directories` |
...
| `test_cli.py` | 17 | output format found/not found, `result.json` written, exit codes, `--json`, flag overrides reach `Settings`, `--max-occurrences` reaching config and rejecting `0`/`-2` |
| `test_api.py` | 29 | see [api](modules/api.md#testing-the-api) |
