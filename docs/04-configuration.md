...
| `DL_MOUTH_MOVEMENT__ENABLED` | `mouth_movement.enabled` | `True` | `bool` | Run the mouth-movement check, which settles the onscreen verdict. Needs the face check enabled (it crops to that detector's boxes) but not a face in the reported frame: a line can open on a title card and cut to the speaker, so this stage scans the whole window and can overturn the face check either way. |
