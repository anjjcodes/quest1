...
class FakeDownloader:
    def __init__(self, video_path: Path, error: Exception | None = None):
        self.video_path = video_path
        self.error = error
        self.calls = 0

    def fetch(self, source, dest_dir, progress=None, reuse_existing=True):
        self.calls += 1
        if self.error:
            raise self.error
        if progress:
            progress(ProgressEvent(PipelineStage.DOWNLOAD, "fake download", 1.0))
...
def test_found_end_to_end(settings, sample_video, tmp_path):
    p, tr = make_pipeline(settings, sample_video, TRANSCRIPT, verifiers=[ScriptedVerifier(VerificationStatus.CONFIRMED, shift=-0.02)])
    ...
    assert tr.warmed == 1
    # timings + progress
    for stage in ("input", "download", "audio", "transcription", "verification", "frame", "total"):
...
def test_not_found_reports_near_misses(settings, sample_video):
    p, tr = make_pipeline(settings, sample_video, "the weather is nice and my mind wanders a lot today")
    result = p.run(PipelineRequest(source=str(sample_video), dialogue=DIALOGUE))
